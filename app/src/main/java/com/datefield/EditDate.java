...
public class EditDate extends AppCompatEditText {
    ...
    public void setDateSeparator(char separator) {
        if (!DefaultSettings.validateSeparator(separator)) {
            Log.i(TAG, "format must be 0, 1 or 2. Use constants from DefaultSettings");
            return;
        }
    ...
    private void resetChars() {
        ...
        separatorPosition[0] = 2;
        separatorPosition[1] = 5;
        if (dateViewFormat == DefaultSettings.DATE_VIEW_FORMAT_YMD) {
            separatorPosition[0] = 4;
            separatorPosition[1] = 7;
        }
        for (int i = 0; i < 10; i++) {
    ...
    public static class DefaultSettings {
        ...
        public static void setDefaultDayPlaceholder(char dayPlaceholder) {
            if (validatePlaceholderChar(dayPlaceholder)) {
                ...
                Log.i(TAG, "only letters (a-zA-Z) are allowed for a placeholder");
            }
        }
